    ...
    if (canOpenFridge && backendUrl) {
      if (!isFridgeOpen) {
        try {
          console.log(
            'Making request to:',
            `${backendUrl}/get_fridge_contents?fridgeID=${fridge.fridgeID}`,
          );
          ...
          if (response.data.success) {
            ...
            alert(
              'Failed to load fridge contents: ' +
          ...
          alert(
            'Network error loading fridge contents. Please check if the backend is running.',
          );
      ...
      alert(
        'Backend URL not yet discovered. Please wait a moment and try again.',
      );
    }
  };
  ...
  const addFoodItem = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert(
          'Error adding food item: ' + (response.data.error || 'Unknown error'),
        );
      }
    } catch (error) {
  ...
  const deleteFoodItem = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `${backendUrl}/delete_fridge_content`,
        {
          data: {
            fridgeID: fridge.fridgeID,
            itemID: id,
      ...
      if (response.data.success) {
        ...
        alert(
          'Error deleting item: ' + (response.data.error || 'Unknown error'),
        );
      }
    } catch (error) {
  ...
  const updateFoodItem = async () => {
    ...
    try {
      const response = await axios.patch(
        `${backendUrl}/update_fridge_content`,
        {
          fridgeID: fridge.fridgeID,
      ...
      if (error.response && error.response.data && error.response.data.error) {
        ...
        alert(
          'Network error updating food item. Please check your connection.',
        );
      }
    }
  };
          ...
          <div>
            ...
            <button
              ...
              onClick={() => {
                ...
                axios
                  .delete(
                    `${backendUrl}/remove_fridge?fridgeID=${fridge.fridgeID}`,
                  )
                  .then((response) => {
                    if (response.data.success) {
                ...
                <>
                  <div className={styles.actionButtons}>
                    <button
                      style={{ width: '150px' }}
                      onClick={() => {
                        mode === 'add' ? setMode('none') : setMode('add');
                      }}
                    ...
                    <button
                      style={{ width: '150px' }}
                      onClick={() => {
                        mode === 'share' ? setMode('none') : setMode('share');
                      }}>
            ...
            <div className={styles.fridgeDetailsContainer}>
              {foods.length == 0 ? (
                <div className={styles.emptyFridgeContainer}>
                  <p className={styles.emptyFridgeMessage}>
                    Looks like there's nothing in your fridge!
                  </p>
                </div>
              ) : (
                <div>
                          ...
                          if (selectedOption) {
                            const sortedFoods = [...foods].sort((a, b) => {
                              ...
                              if (selectedOption === 'expirationDate') {
                                ...
                                return (
                                  Date.parse(a[selectedOption]) -
                                  Date.parse(b[selectedOption])
                                );
                              }
                              ...
                              return String(a[selectedOption]).localeCompare(
                                String(b[selectedOption]),
                              );
                            });
                      ...
                      .map((food) => (
                        <div
                          ...
                          style={
                            food.isInFreezer
                              ? { backgroundColor: '#33f' }
                              : Date.parse(food.expirationDate) < Date.now()
                              ? { backgroundColor: '#a33' }
                              : {}
                          }>
                    ...
                    <div className={styles.userInput}>
                      ...
                      <div className={styles.inlineForm}>
                        ...
                        <input
                          ...
                          title={
                            isInFreezer
                              ? 'Expiration date not required for freezer items'
                              : ''
                          }
                        />
